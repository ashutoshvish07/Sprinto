...
import { Avatar, Badge, ConfirmDialog } from './ui'
...
export default function TaskCard({ task, onUpdate, onDelete, compact = false }) {
  ...
  if (compact) {
    return (
      <div
        onClick={goToDetail}
        className="flex items-center gap-3 p-3 bg-slate-800/40 border border-slate-700/40 rounded-xl hover:border-slate-600/50 transition-all group cursor-pointer"
      >
  ...
  return (
    <>
      <div
        onClick={goToDetail}
        className="bg-slate-800/60 border border-slate-700/50 rounded-xl p-4 hover:border-indigo-500/40 hover:bg-slate-800/80 transition-all group cursor-pointer"
      >
        <div className="flex items-start justify-between gap-2 mb-3">
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="opacity-0 group-hover:opacity-100 transition-opacity">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" className="w-3.5 h-3.5 text-slate-500">
